...
public class Admindao {
	...
	public int checkLogin(String id, String pass)
	{
		...
		ResultSet rs = null;
		String sql = "select pass from guest where id=? and admin=1";
	...
	/////// 모든 회원의 정보를 가져옴 //////
	public ArrayList<GuestDTO> listMember()
	{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	/*public ArrayList<PointManagerDTO> LoadPointLog()
	{
		...
		try {
			...
			while(rs.next())
			{
				dto = new PointManagerDTO(rs.getString("guest_id"), rs.getInt("p_calcul"),
										rs.getString("p_cont"), rs.getDate("p_date"));
