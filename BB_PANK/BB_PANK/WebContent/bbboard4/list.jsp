	...
	<section>
		...
		<table>
			<tr>
				...
				<a
					href="${pageContext.request.contextPath}/bbboard4/list.do?bn=4&prefaces=0">[전체목록]
				</a> <a
					href="${pageContext.request.contextPath}/bbboard4/list.do?bn=4&prefaces=1">[연예]
				</a> <a
					href="${pageContext.request.contextPath}/bbboard4/list.do?bn=4&prefaces=2">[정치]
				</a> <a
					href="${pageContext.request.contextPath}/bbboard4/list.do?bn=4&prefaces=3">[스포츠]
				</a> <a
					href="${pageContext.request.contextPath}/bbboard4/list.do?bn=4&prefaces=4">[뻘글]
				</a></td>
			</tr>
		</table>
		<table class="listwritebutton">
			<tr>
				<td><a
					href="${pageContext.request.contextPath}/bbboard4/writeForm.do?">글쓰기</a>
				</td>
			</tr>
		</table>
		<c:if test="${count == 0}">
				...
				<c:forEach var="article" items="${articleList}">
					<tr>
						...
						<td class="titletd"><c:if test="${article.depth > 0}">
								<img
									src="${pageContext.request.contextPath}/bbboard4/images/level.gif"
									width="${5 * article.depth}">
								<img
									src="${pageContext.request.contextPath}/bbboard4/images/re.gif">
							</c:if> <c:if test="${article.depth == 0}">
								<img
									src="${pageContext.request.contextPath}/bbboard4/images/level.gif"
									width="${5 * article.depth}">
							</c:if> <a
							href="${pageContext.request.contextPath}/bbboard4/content.do?num=
			${article.num}&pageNum=${currentPage}&bn=4">
								${article.subject}</a> <c:if test="${article.readcount >= 20}">
								<img
									src="${pageContext.request.contextPath}/bbboard4/images/hot.gif">
							</c:if></td>
						<!-- 작성자 -->
		...
		<c:if test="${count > 0}">
			...
			<c:if test="${startPage > pageBlock}">
				...
				<a
					href="${pageContext.request.contextPath}/bbboard4/list.do?pageNum=
${startPage - pageBlock }&prefaces=${prefaces}">이전</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a
					href="${pageContext.request.contextPath}/bbboard4/list.do?pageNum=${i}&prefaces=${prefaces}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount}">
				<a
					href="${pageContext.request.contextPath}/bbboard4/list.do?pageNum=${startPage + pageBlock }&prefaces=${prefaces}">다음</a>
			</c:if>
		</c:if>
		<p>
			<!-- 검색기능  -->
		<form method=post name=search
			action="${pageContext.request.contextPath}/bbboard4/list.do?&prefaces=${prefaces}&bn=${bn}"
