...
<table class="listwritebutton">
	<tr>
		<td>
			<a href="${pageContext.request.contextPath}/bbboard3/writeForm.do?pageNum=${pageNum}&bn=${bn}">글쓰기</a>
		</td>
	</tr>
</table>
<c:if test="${count == 0}">
...
<table class="listtable">
	...
	<tr>
		...
		<td class="titletd">
			...
			<a href="${pageContext.request.contextPath}/bbboard3/content.do?num=${article.num}&pageNum=${pageNum }&bn=${bn}">
				${article.subject}</a>
...
<c:if test="${count > 0}">
	...
	<c:if test="${startPage > pageBlock}">
		<a href="${pageContext.request.contextPath}/bbboard3/list.do?pageNum=${startPage - pageBlock }&bn=${bn}">이전</a>
	</c:if>
	
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<a href="${pageContext.request.contextPath}/bbboard3/list.do?pageNum=${i}&bn=${bn}">[${i}]</a>
	</c:forEach>
	
	<c:if test="${endPage < pageCount}">
		<a href="${pageContext.request.contextPath}/bbboard3/list.do?pageNum=${startPage + pageBlock }&bn=${bn }">다음</a>
	</c:if>
</c:if>
</section>
<div id="footer">
